...
export const useProjectsStore = defineStore("projects", () => {
  ...
  const getProjectById = async (id: string): Promise<Project | undefined> => {
    ...
    try {
      ...
      error.value =
        err instanceof Error ? err.message : "Unable to fetch project";
      return undefined;
    } finally {
      loading.value = false;
    }
  ...
  const fetchProjectTeamMembers = async (projectId: string): Promise<any[]> => {
    try {
      ...
      const teamResponse = await api.get(`/teams/${projectData.teamId}/users`);
      return teamResponse.users || [];
    } catch (err) {
      error.value =
        err instanceof Error ? err.message : "Unable to fetch team members";
      return [];
    }
  };
