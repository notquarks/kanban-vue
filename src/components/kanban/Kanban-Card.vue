...
import { Calendar, CheckSquare, Paperclip, Tag } from "lucide-vue-next";
  ...
  <div class="flex w-full flex-col">
    <DialogRoot>
      <DialogTrigger as-child :disabled="isDragging">
        <div
          :class="[
            'mt-1 flex h-fit w-full cursor-grab flex-col gap-2 rounded border border-gray-400/80 bg-white px-2 py-1.5 shadow transition-all duration-200 hover:bg-gray-300/80 active:cursor-grabbing',
            statusColor,
            { 'cursor-not-allowed opacity-50': cardData?.status === 'done' },
        ...
        >
          <div v-if="labels.length > 0" class="flex flex-wrap gap-1">
            <div
              ...
              :key="label.id"
              class="rounded-sm px-2 py-0.5 text-[10px] font-medium text-white"
            ...
            <div
              ...
              class="flex items-center gap-0.5 rounded-sm bg-gray-200 px-2 py-0.5 text-[10px] font-medium text-gray-600"
            >
              <Tag :size="10" />
              +{{ labels.length - 3 }}
            </div>
          ...
          <h3 class="text-sm font-medium">{{ cardData?.title }}</h3>

          <div
            v-show="
              cardData?.members?.length ||
              todosCount > 0 ||
              attachmentsCount > 0 ||
              cardData?.dueDate
            "
            class="flex items-center justify-between border-t border-gray-100 pt-1 text-xs text-gray-600"
          >
            <div
              v-show="cardData?.members?.length"
              class="flex items-center gap-1"
            >
              <div class="flex flex-row -space-x-1">
                <div
                  v-for="member in cardData?.members?.slice(0, 3)"
                  :key="member.id"
                  class="relative"
                >
                  <AvatarRoot
                    class="inline-flex h-5 w-5 items-center justify-center overflow-hidden rounded-full bg-gray-700 align-middle ring-1 ring-white select-none"
                  >
                    <AvatarFallback
                ...
                <div
                  v-if="cardData?.members && cardData.members.length > 3"
            ...
            <div class="ml-auto flex items-center gap-2">
              ...
              >
                ...
                <span class="text-xs font-medium"
                  >{{ completedTodosCount }}/{{ todosCount }}</span
                >
              </div>
