...
import type { AuthVariables, SafeUser, JWTPayload } from "../middleware/auth";
...
authRoutes.post(
  ...
  validator("json", (value, c) => {
    ...
    if (!passwordValidation.valid) {
      return c.json(
        { error: passwordValidation.message } as ErrorResponse,
        400,
      );
    }
...
authRoutes.get("/me", jwtMiddleware(), requireAuth(), async (c) => {
...
authRoutes.post("/logout", jwtMiddleware(), requireAuth(), async (c) => {
...
authRoutes.post(
  ...
  validator("json", (value, c) => {
    ...
    if (!body.currentPassword?.trim()) {
      return c.json(
        { error: "Current password is required" } as ErrorResponse,
        400,
      );
    }
    ...
    if (!body.newPassword?.trim()) {
      return c.json(
        { error: "New password is required" } as ErrorResponse,
        400,
      );
    }
