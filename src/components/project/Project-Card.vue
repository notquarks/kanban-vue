...
<template>
  <div
    class="group/card flex aspect-square w-auto min-w-64 cursor-pointer flex-col rounded-md border bg-white p-4 shadow-md transition-all duration-100 ease-linear hover:bg-gray-700 hover:text-gray-100 hover:shadow-lg"
  ...
  >
    ...
    <div class="flex flex-row">
      <div class="flex-1">
        <h3
          class="truncate font-semibold text-gray-900 group-hover/card:text-gray-50"
        >
          {{ project.name }}
        </h3>
        <p
          class="mt-1 line-clamp-2 text-sm text-gray-600 group-hover/card:text-gray-100"
        >
      ...
      <DropdownMenuRoot v-model:open="toggleState">
        <DropdownMenuTrigger
          @click.stop
          class="rounded-xs p-0.5 hover:cursor-pointer hover:bg-gray-500"
        >
          <EllipsisVertical />
        </DropdownMenuTrigger>
        <DropdownMenuPortal>
          ...
          >
            ...
            <AlertDialogRoot>
              ...
              <AlertDialogPortal>
                <AlertDialogOverlay
                  class="data-[state=open]:animate-overlayShow fixed inset-0 z-30 bg-gray-800/90"
                />
